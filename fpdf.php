...
/**
 * This plugin adds Olivier Plathey's FPDF class to WordPress. The class is
 * entirely his work. All I have added are constants for defining name, version,
 * and paths, and activation hooks.
 * 
 * Plugin Name: CK FPDF Library
 * Plugin URI:  https://github.com/ChristoferKoch/CK-FPDF
 * Description: A PHP class which allows to generate PDF files with pure PHP http://www.fpdf.org/
 * Version:     1.0.0
 * Author:      Christofer Koch
 * Author URI:  http://www.fpdf.org/
 * License:     Permissive
 * License URI: ./license.txt
 */

// Define constants
defined('CK_FPDF_PLUGIN_NAME') or define('CK_FPDF_PLUGIN_NAME', 'ck_fpdf');
defined('CK_FPDF_PLUGIN_VERSION') or define('CK_FPDF_PLUGIN_VERSION', '1.0.0');
defined('CK_FPDF_BASE_NAME') or define('CK_FPDF_BASE_NAME', plugin_basename(__FILE__));
defined('CK_FPDF_ROOT_PATH') or define('CK_FPDF_ROOT_PATH', plugin_dir_path(__FILE__));
defined('CK_FPDF_ROOT_URL') or define('CK_FPDF_ROOT_URL', plugin_dir_url(__FILE__));
...
class ckFPDF {
	...
	public static function activation() {
		if (!ckFPDF::php_version_check()) {

			// Deactivate the plugin
			deactivate_plugins(__FILE__);

			// Throw an error in the wordpress admin console
			$error_message = __('This plugin requies PHP version 5.1 or newer', 'ck_fpdf');
			die($error_message);
		}
	}
...
register_activation_hook(__FILE__, array('ckFPDF', 'activation'));

function ckfdf_load_plugin() {
	new ckFPDF();
}

add_action('plugins_loaded', 'ckfdf_load_plugin');
